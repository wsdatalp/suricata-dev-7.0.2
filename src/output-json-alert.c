        ...
        uint16_t flags, JsonAddrInfo *addr, char *xff_buffer)
{
    ...
    jb_set_uint(js, "groupId", pa->s->gid);
    jb_set_uint(js, "signatureId", pa->s->id);
    jb_set_uint(js, "rev", pa->s->rev);
    /* TODO: JsonBuilder should handle unprintable characters like
     * SCJsonString. */
    jb_set_string(js, "signatureMsg", pa->s->msg ? pa->s->msg: "");
    jb_set_string(js, "category", pa->s->class_msg ? pa->s->class_msg: "");

    DetectReference *ref = pa->s->references;
    while (ref){
        if (strncmp(ref->reference, "CVE", strlen("CVE")) == 0) {
            jb_set_string(js, "cveCode", ref->reference);
        }
        ref = ref->next;
    }
...
void JsonAlertLogRegister (void)
{
    ...
    //告警流量对应的全流量（上下文流量）输出
    OutputRegisterPacketSubModule(LOGGER_JSON_ALERT, "eve-log", "JsonAlertContextLog",
                                  "eve-log.alert2", JsonAlertLogInitCtxSub, JsonContextAlertLogger,
                                  JsonAContextCondition, JsonAlertLogThreadInit, JsonAlertLogThreadDeinit,
                                  NULL);
}
