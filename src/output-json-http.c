...
static void EveHttpLogJSONBasic(JsonBuilder *js, htp_tx_t *tx)
{
    ...
    if (tx->request_hostname != NULL) {
        jb_set_string_from_bytes(
                js, "host", bstr_ptr(tx->request_hostname), bstr_len(tx->request_hostname));
    }
    ...
    if (tx) {
        ...
        if (htud != NULL) {
            BodyPrintableBuffer(js, &htud->request_body, "httpRequestBody");
            BodyPrintableBuffer(js, &htud->response_body, "httpResponseBody");
        }
    }
    /* content-type */
    if (tx->response_headers != NULL) {
        ...
        if (h_content_type != NULL) {
            ...
            if (p != NULL)
                *p = '\0';
            jb_set_string(js, "contentType", string);
        }
...
static void EveHttpLogJSONExtended(JsonBuilder *js, htp_tx_t *tx)
{
    ...
    if (tx->request_method != NULL) {
        jb_set_string_from_bytes(
                js, "method", bstr_ptr(tx->request_method), bstr_len(tx->request_method));
    }
