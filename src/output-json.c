        ...
        const char *event_type, JsonAddrInfo *addr, OutputJsonCtx *eve_ctx)
{
    ...
    jb_set_uint(js, "packetLen", GET_PKT_LEN(p));

    unsigned long p_max_len = GET_PKT_LEN(p);
    unsigned long p_len = BASE64_BUFFER_SIZE(p_max_len);
    uint8_t p_encoded_packet[p_len];
    if (Base64Encode((unsigned char*) GET_PKT_DATA(p), p_max_len, p_encoded_packet, &p_len) == SC_BASE64_OK) {
